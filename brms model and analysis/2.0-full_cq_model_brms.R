...
# Fixed-effect draws with plain English names
fixed_draws <- fit1 %>%
  spread_draws(b_Intercept, b_log_Q_sc, b_post, `b_log_Q_sc:post`,
...
# Use ranef() for ecoregion random effects — handles ecoregion names with
# commas reliably (spread_draws with two-index syntax can fail on those).
# Dominant uncertainty in pre→post shifts comes from the fixed-effect draws above.
re_vals <- as.data.frame(ranef(fit1)$na_l3name[, "Estimate", ]) %>%
  rownames_to_column("na_l3name") %>%
  rename(re_intercept = Intercept, re_slope = log_Q_sc) %>%
  left_join(burn_by_group, by = "na_l3name")

# crossing() gives one row per draw × ecoregion
...
# coef() returns total group estimates (population mean + random deviation),
# which is the right quantity — each ecoregion's actual pre-fire intercept and slope.
# NOTE uses posterior medians from the marginal distributions of each parameter, 
# not joint. Trend line is not a formal estimate of ρ
grp_coef <- coef(fit1, probs = c(0.25, 0.75))$na_l3name

grp_params <- left_join(
  grp_coef[,, "Intercept"] %>%
    as.data.frame() %>%
    rownames_to_column("na_l3name") %>%
    rename(int_est = Estimate, int_lo = Q25, int_hi = Q75),
  grp_coef[,, "log_Q_sc"] %>%
    as.data.frame() %>%
    rownames_to_column("na_l3name") %>%
    rename(slope_est = Estimate, slope_lo = Q25, slope_hi = Q75),
  by = "na_l3name"
)
