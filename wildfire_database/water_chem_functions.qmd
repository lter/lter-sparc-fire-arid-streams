...
- Semantics:
  ...
  missing/invalid data still fails strict checks
  - Aggregates per-event severity to grouped-event sets using fire-area weights
  (`firearea.dd_area_stats.fire_catch_area`)
  - Raises an error when any grouped-event set has invalid/missing severity data
...
- Duplicate guards: Applied to ecoregion, elevation, NLCD, and EVI to preserve
base MV row count
...
- Requirements:
  + firearea.evi_join_largest_{analyte} and base MV must exist (validated via
  pg_matviews)
