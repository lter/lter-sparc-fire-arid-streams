...
Critical is that this step is done with the write going to a throwaway database.
...
catchments <- sf::st_read(pg, query = "select * from firearea.catchments ;")
ecoregions <- sf::st_read("aridland_ecoregions.geojson")

texas_eco <- sf::st_join(catchments, ecoregions, join = sf::st_within)
texas_eco <- texas_eco |> dplyr::filter(!is.na(NA_L3CODE))

stream_sites_candidates <- stream_sites_candidates |> # from initial
  dplyr::mutate(usgs_site = paste0("USGS-", site_no)) |>
  dplyr::filter(usgs_site %in% texas_eco$usgs_site)
