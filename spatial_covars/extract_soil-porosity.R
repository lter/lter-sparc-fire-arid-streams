...
## SOIL POROSITY (Brooks-Corey Index)
...
soil_path <- file.path(path, "raw-spatial-data", "polaris_soil", "polaris_lambda-mean-0-5_tiles")
...
porosity_files <- sort(dir(path = soil_path, pattern = ".tif"))

# Make an empty list
porosity_list <- list()

# Loop across each raster tile
for(focal_porosity in porosity_files){
  
  # Progress message
  message("Beginning extraction for raster tile '", focal_porosity, "'")
  
  # Read it in as a raster
  focal_rast <- terra::rast(x = file.path(soil_path, focal_porosity))
  ...
  if(nrow(focal_df) >= 1){ porosity_list[[as.character(focal_porosity)]] <- focal_df }
  
} # Close extraction loop

# Tidy environment
rm(list = setdiff(x = ls(), y = c("sf_file", "group_cols", "path", "soil_path", "porosity_list")))
...
for(focal_site in sort(unique(sf_file$usgs_site))){
  ...
  for(k in 1:length(porosity_list)){
    
    # Grab list element & subset to just this focal site
    focal_porosity_data <- porosity_list[[k]] %>% 
      dplyr::filter(usgs_site == focal_site)
    
    # Add this to the list for this site
    site_list[[paste0(focal_site, "-", k)]] <- focal_porosity_data
  ...
  site_df <- site_list %>% 
    ...
    dplyr::summarize(soil_porosity_brooks.corey_avg = mean(value, na.rm = T),
                     soil_porosity_brooks.corey_median = median(value, na.rm = T),
                     soil_porosity_brooks.corey_min = min(value, na.rm = T),
                     soil_porosity_brooks.corey_max = max(value, na.rm = T)) %>%
    dplyr::ungroup()
  
  # Add to larger list
  porosity_v0[[paste(focal_site)]] <- site_df
  
} # Close outer loop

# Process that output
porosity_v1 <- porosity_v0 %>% 
  purrr::list_rbind(x = .)

# Check structure
dplyr::glimpse(porosity_v1)
...
final_porosity <- porosity_v1

# Create folder to export to
dir.create(path = file.path(path, "extracted-data"), showWarnings = F)

# Define file path for CSV
porosity_path <- file.path(path, "extracted-data", "fire-arid_soil-porosity.csv")

# Export the summarized data
write.csv(x = final_porosity, na = '', row.names = F, file = porosity_path)

# Upload to GoogleDrive
googledrive::drive_upload(media = porosity_path, overwrite = T,
